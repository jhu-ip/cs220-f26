...
reference solutions:
  ...
  2. `FILE *fp = fopen("img.dat", "wb"); if (fp == NULL) { perror("Error opening file for writing");  return 1; } fwrite(ray1, sizeof(int), 1000, fp); fclose(fp);` to do the writing and `fp = fopen("img.dat", "rb"); if (fp == NULL) { perror("Error opening file for reading"); return 1; } fread(ray2, sizeof(int), 1000, fp); fclose(fp);` to do the reading.
