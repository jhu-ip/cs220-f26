...
reference solutions:
  ...
  3. int to double is fine, it will get promoted. double to int is compiler warning(?) and should be avoided or explicitly typecast first; data is likely to be lost.
  4. values from primitive type function call arguments are copied into the parameter variables when the call is executed
  5. [NEW] Inside update_values`:` x = 15, y = 14 
