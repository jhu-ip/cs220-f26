...
Start by going to the following website in a web browser: <https://www.onlinegdb.com/online_c_compiler> (right click to open in a new tab).
...
We're using this online programming environment for this exercise in order to make sure that everyone can start working on C programs right away, even if they don't yet have an account on the CS ugrad machines.  All subsequent exercises (and assignments) will be done on the ugrad machines, starting with the next class session.
